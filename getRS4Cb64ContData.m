...
for j=1:length(blocks)
  ...
  parfor i = 1:length(blockNames)
    ...
    for ch = 1:totPolytrodes
      ...
      % Make save directory if it dosen't exist 
      currentpath = [savepath,blockNames(i).name(1:11),'_DAT_files\Cb\Polytrode_',num2str(poly-1),'\'];
      if ~exist(currentpath, 'dir')
        mkdir(currentpath);
      end
      ...
      su_Cb = su_Cb(:)';
      fileID = fopen([currentpath,'SU_CONT_Cb_poly_',num2str(tet-1),'_',num2str(i-1),'.dat'],'w');
